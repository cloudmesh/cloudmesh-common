###############################################################
# pytest -v --capture=no tests/test_stopwatch_mllog_2.py
# pytest -v --capture=no tests/test_stopwatch_mllog_2.py::Test_stopwatch.test_001
# pytest -v  tests/test_stopwatch_mllog_2.py
###############################################################
import platform
import os
import time
...
class Test_Printer:
    ...
    def test_check_loading(self):
        ...
        if platform.system() == 'Windows':
            assert "cloudmesh-common\\\\tests\\\\test_stopwatch_mllog_2.py" in line
            assert "cloudmesh-common\\\\cloudmesh\\\\common\\\\StopWatch.py" not in line
        else:
            assert "cloudmesh-common/tests/test_stopwatch_mllog_2.py" in line
            assert "cloudmesh-common/cloudmesh/common/StopWatch.py" not in line
    ...
    def test_stopwatch_submission(self):
        HEADING()
        clean()
        StopWatch.activate_mllog()
        data = yaml.safe_load(submission_config)
        StopWatch.organization_submission(**data)
        ...
        assert '"event_type": "POINT_IN_TIME", "key": "submission_platform"' in content
        assert '"value": "rivanna"' in content
        assert '"event_type": "POINT_IN_TIME", "key": "submission_version"' in content
        assert '"value": "mlcommons-earthquake-v1.0"' in content
        assert '"event_type": "POINT_IN_TIME", "key": "github_commit_version"' in content
